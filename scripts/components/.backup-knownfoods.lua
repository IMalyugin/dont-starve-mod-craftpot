...
function KnownFoods:_Decomposition(mix)
  local recipe = {minnames={},maxnames={},mintags={},maxtags={}}
  local list = {}
  self:_DecompositionStep(mix, recipe, list)
  return list
end
...
function KnownFoods:_Composition(list) -- only compose 1 level, because otherwise it will be too complicated for an ordinary person comprehension
  local mix = {{}} -- initially an OR
  local sets = {} -- specific format {type:'name'/'tag'}_{name/tag}_{amt} = {cnt={times_found},name/tag={name/tag},amt={amt}}
  -- first read the recipes in a huge line
  for idx,recipe in ipairs(list) do
    local branch = {}
    for name, amt in pairs(recipe.minnames) do
    ...
    for tag, amt in pairs(recipe.mintags) do
  ...
  local uniques = {}
  for alias, data in pairs(sets) do
    if #data.used > 1 then
      table.insert(uniques, data)
    else
      table.insert(mix[0], data.mix)
    end
  end

  if #uniques > 0 then
    -- let us begin composition
    for idx, data in ipairs(uniques) do
      if #uniques == #data.used then
      end
    end
  end
...
function KnownFoods:OnAfterLoad(config)
  ...
  --[[local testRecipes = require "testrecipes"
  for foodname, testRecipe in pairs(testRecipes) do
    print(foodname..": ")
    for num, recipe in ipairs(self:_Decomposition(testRecipe.minmix)) do
      print("~"..num..".")
      for name,amt in pairs(recipe.minnames) do
        print("~~"..name.."="..amt)
      end
      ...
      --self._knownfoods[foodname] = listRecipe
    end]]
  ...
  for cookername,recipes in pairs(self._Cooking.recipes) do
    for foodname,recipe in pairs(recipes) do
      local metatable = {__index = function (t,k) print(k) return 1 end}
      local test = {}
      setmetatable(test, metatable)
      recipe.test('',test,test)
      --[[local success, err = pcall(recipe.test)
      if not success then
        local start = string.find(err, 'attempt')
        print(string.sub(err,start))
      end]]
 ...
 -- try to replace minname it with a new name
    for name,_ in pairs(self._allnames) do
      if name ~= minname then
        ...
        if self:_Test(foodname,minnames,mintags) then
